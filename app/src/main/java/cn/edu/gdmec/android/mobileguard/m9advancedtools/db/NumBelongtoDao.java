    ...
    public static String getLocation(Context context, String phonenumber) {
        ...
        if (phonenumber.matches("^1[34578]\\d{9}$")) {
            Cursor cursor = db.rawQuery(
                    "select location from data2 where id = (select outkey from data1 where id = ?)", new String[]{phonenumber.substring(0, 7)});
            ...
            switch (phonenumber.length()) {
                ...
                default:
                    if (location.length() >= 9 && location.startsWith("0")) {
                        String address = null;
                        Cursor cursor = db.rawQuery("select locatino from data2 where area = ?",
